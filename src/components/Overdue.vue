...
mounted(){
   this.users = this.$store.state.Users.filter((item)=>item.paidStatus == "Overdue")
}
  ,
...
<template>
<div class="MainContent w-[900px] rounded-2xl">
        ...
        <tbody>
          <tr v-for="user in users" :key="user.email" @click="Alrt">
            ...
            <td class="">
              ...
              <td class="">
                <span v-bind:class="{
                  'bg-[#E6E6F2]  px-[4px] rounded-full text-[#6767FD] w-[80px] h-[23px] font-bold flex gap-[2px] ': user.online == 'Active',
                  'bg-[#F2F0F9]  px-[4px] rounded-full text-[#B0ACC6] w-[80px] h-[23px] font-bold flex gap-[2px]': user.online == 'Inactive'


                }"> <b class="text-[13px] font-extrabold px-[2px]">.</b><span class="font-bold">{{user.online}}</span></span>
              ...
              <td class="">
                  <span v-bind:class="{
                  'bg-[#CDFFCD]  px-[4px] rounded-full text-[#0D870D] w-[80px] h-[23px] font-bold flex gap-[2px] ': user.paidStatus == 'Paid',
                  'bg-[#FFECCC]  px-[4px] rounded-full text-[#965E00] w-[80px]  h-[23px] font-bold flex gap-[2px]': user.paidStatus == 'Unpaid',
                    'bg-[#FFE0E0]  px-[4px] rounded-full text-[#D81C1C] w-[80px] h-[23px] font-bold flex gap-[2px]': user.paidStatus == 'Overdue'



                }">
               <b class="text-[14px] font-extrabold ">.</b><span class="font-bold">{{user.paidStatus}}</span></span>
